...
class DGT(nn.Module):
    ...
    def __init__(
        ...
        print('Layer 2 combines the predicates')
        ...
        if verbose:
            print(self._and_l.weight)
            print(self._and_l.bias)
    ...
    def forward(
        ...
        if self.verbose:
            print('sparse')
            print(and_a_hard)
        self.selected_experts = F.softmax(and_a_hard, dim=-1) # which leaf is selected for each example

        ########### L3 ########## dot product with regression layer
        ...
        return or_z, None # no softmax in the output
