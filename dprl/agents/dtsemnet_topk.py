    ...
    def forward(
        ...
        if self.is_regression: # for regression
            ...
            if self.linear_control:
                ...
                # Process each expert only on its corresponding inputs
                ...
                for expert_id in range(len(self.experts)):
                    # Find which entries require this expert
                    mask = (flat_expert_inds == expert_id)
                    if mask.any():
                        selected_inputs = flat_inputs[mask]
                        selected_outputs = self.experts[expert_id](selected_inputs)
                        # print('selected_outputs', selected_outputs)
                        outputs[mask] = selected_outputs
                        # print('outputs', outputs)

                        # Extract corresponding log std for this expert
                        log_std = self.log_stds[:, expert_id]  # shape: [output_dim]
                        log_stds[mask] = log_std.unsqueeze(0).expand(mask.sum(), -1)  # broadcast over batch
