...
# Evaluation function
def evaluate(model, dataloader, loss_fn, use_gpu=False):
    ...
    with torch.no_grad():  # Disable gradient calculation for evaluation
        for data, target in dataloader:
            ...
            total_loss += loss
            batches += 1
    rmse = torch.sqrt(total_loss / batches).item() 
    return total_loss / batches, rmse 
...
if __name__ == "__main__":
    ...
    if mname == "mlp":
        from src.sup_configs_mlpreg import get_config, load_dataset, real_dataset_list
    elif mname == "dtregnet":
        from src.sup_configs_dtregnet import get_config, load_dataset, real_dataset_list
    elif mname == "dgt":
        from src.sup_configs_dgtreg import get_config, load_dataset, real_dataset_list
    

    command_line = str(args)
    command_line += "\n\npython -m reg_dataset.dtregnet_train " + " ".join(
    ...
    for dataset_id, data_config in enumerate(data_configs):
        ...
        X_train, y_train, X_val, y_val, X_test, y_test = load_dataset(data_config, standardize=args["should_normalize_dataset"]) 
        ...
        for simulation in range(simulations):
            ...
            if data_config["lr_scheduler"]:
                ...
                # scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=len(train_dataloader), eta_min=0.001,)
            else:
                scheduler = None
            ...
            for epoch in range(epochs):
                ...
                if args["verbose"]:
                    ...
                    print(f"Epoch: {epoch+1}/{epochs}, Average Loss: {average_loss:.4f}, Test Acc: {multiv_acc_test:.4f}")
