...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_path', help='which model file to load and test', type=str, default='best_model')
    ...
    # [40, 50, 60, 70, 80]
    for mod_seed in [11, 13, 17, 19, 23]:
        # load model
        model = SAC.load(f'{args.model_path}best_model_seed{mod_seed}', device=args.device)
        ...
        print(f'model_{mod_seed}: {test_reward_mean} +/- {test_reward_std}')
    ...
    with open(f'{args.model_path}test_results.txt', 'w') as f:
        ...
        f.write(f'{np.round(overall_mean, 1)}, {np.round(overall_std, 2)}, {np.round(sem, 2)}\n')
