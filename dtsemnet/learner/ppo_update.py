...
class PPO:
    def __init__(self, actor_critic_arr, two_nets=True, use_gpu=False, agent=None, random=None):
        ...
        if two_nets:
            ...
            if self.actor.input_dim == 4: #cart and acrobot
                self.actor_opt = optim.RMSprop(self.actor.parameters(), lr=2e-2)
                self.critic_opt = optim.RMSprop(self.critic.parameters(), lr=2e-2)
                
            elif self.actor.input_dim == 6: # for acrobot
                self.actor_opt = optim.RMSprop(self.actor.parameters(), lr=0.01)
                self.critic_opt = optim.RMSprop(self.critic.parameters(), lr=0.01)
            
            elif self.actor.input_dim == 8: # for lunar
                self.actor_opt = optim.RMSprop(self.actor.parameters(), lr=2e-2)
                self.critic_opt = optim.RMSprop(self.critic.parameters(),lr=2e-2)
                
            elif self.actor.input_dim == 37: #zerlings
                self.actor_opt = optim.RMSprop(self.actor.parameters(),
                                                lr=2e-2)
                self.critic_opt = optim.RMSprop(self.critic.parameters(),
                                                    lr=2e-2)
            else:
                self.actor_opt = optim.RMSprop(self.actor.parameters(),
                                                lr=2e-2)
        ...
        for iteration in range(num_iters):
            ...
            for b in range(batch_size):
                ...
                label = torch.LongTensor([
                    heuristic_teacher.get_action(
                        state[0].detach().clone().data.cpu().numpy()[0])
                ])
                action_loss = torch.nn.functional.cross_entropy(
                    new_action_probs, label)
    ...
    def batch_updates(self, rollouts, agent_in, go_deeper=False):
        ...
        for iteration in range(num_iters):
            ...
            if len(samples) <= 1:
                continue
            state = torch.cat([sample['state'][0] for sample in samples],
                              dim=0)
            ...
            action_taken = torch.Tensor(
                [sample['action_taken'] for sample in samples])
            if self.use_gpu:
            ...
            if samples[0]['hidden_state'] is not None:
                actor_hidden_state_batch0 = torch.cat(
                    [sample['hidden_state'][0][0] for sample in samples],
                    dim=1)
                actor_hidden_state_batch1 = torch.cat(
                    [sample['hidden_state'][0][1] for sample in samples],
                    dim=1)
                actor_hidden_state = (actor_hidden_state_batch0,
                                      actor_hidden_state_batch1)
                critic_hidden_state_batch0 = torch.cat(
                    [sample['hidden_state'][1][0] for sample in samples],
                    dim=1)
                critic_hidden_state_batch1 = torch.cat(
                    [sample['hidden_state'][1][1] for sample in samples],
                    dim=1)
                critic_hidden_state = (critic_hidden_state_batch0,
                                       critic_hidden_state_batch1)
            ...
            if go_deeper:
                deep_action_probs = torch.Tensor(
                    [sample['deeper_action_prob'] for sample in samples])
                deep_adv = torch.Tensor(
                    [sample['deeper_advantage'] for sample in samples])
                deeper_old_probs = torch.cat([
                    sample['deeper_full_prob_vector'].unsqueeze(0)
                ...
                deeper_action_indices = [
                    int(action_ind.item()) for action_ind in action_taken
                ]
                deeper_val = new_deep_vals[np.arange(0, len(new_deep_vals)),
                                           deeper_action_indices]
                ...
                for weight_index in range(len(self.actor.layers)):
                    new_act_weight = torch.Tensor(
                        self.actor.layers[weight_index].cpu().data.numpy())
                    new_act_comp = torch.Tensor(
                        self.actor.comparators[weight_index].cpu().data.numpy(
                        ))
