'''Implementation of ProLonet agent.
Code reused from ProLoNets Github'''
...
from agents.prolonet.converter import init_cart_nets, swap_in_node, add_level, \
    init_lander_nets, init_micro_net, init_adversarial_net, init_acrobot_nets, init_sc_nets, \
...
class DeepProLoNet:
    def __init__(self,
        ...
        self.bot_name = f'prolo_{"gpu" if self.use_gpu else "cpu"}'
        if vectorized:
            self.bot_name += '_vect'
        if randomized:
            self.bot_name += '_rand'
        ...
        #         distribution, use_gpu, vectorized, randomized)
            ...
            self.env_name = 'fdz' # FindAndDefeatZerglings
        ...
        #     self.action_network, self.value_network = init_sc_build_hellions_net(
        #         distribution, use_gpu, vectorized, randomized)
        # elif input_dim == 6 and output_dim == 5:  # Fire Sim
        #     self.action_network, self.value_network = init_fire_nets(
        #         distribution, use_gpu, vectorized, randomized,
        #         self.bot_name.split('_')[0])
        elif input_dim == 6 and output_dim == 3:  # Lunar Lander
            self.env_name = 'acrobot'
            self.lr = 2e-2
            self.action_network, self.value_network = init_acrobot_nets(
                distribution, use_gpu, vectorized, randomized)
        ...
        if self.deepen:
            ...
            self.deeper_actor_opt = torch.optim.RMSprop(
                self.deeper_action_network.parameters(), lr=self.lr)
            self.deeper_value_opt = torch.optim.RMSprop(
                self.deeper_value_network.parameters(), lr=self.lr)
        else:
    ...
    def buffer_insert(self, state=None, action=None, reward=None):
        self.replay_buffer.insert(
            ...
            deeper_value_pred=self.last_deep_value_pred[
    ...
    def end_episode(self):
        value_loss, action_loss = self.ppo.batch_updates(self.replay_buffer,
                                                         self,
                                                         go_deeper=self.deepen)
        self.num_steps += 1
    ...
    def save(self, fn='trained_models/', ix=0):
        ...
        dir_path = f'{fn}{self.path}'

        if not os.path.exists(dir_path):
            os.makedirs(dir_path)
        act_fn = f'{dir_path}seed{ix}_actor.tar'

        save_prolonet(act_fn, self.action_network)
        if self.deepen:
            deep_act_fn = f'{dir_path}seed{ix}_deep_actor.tar'
            save_prolonet(deep_act_fn, self.deeper_action_network)


    def load(self, act_fn=None, deep_act_fn=None):

        if os.path.exists(act_fn):
            self.action_network = load_prolonet(act_fn, self.use_gpu)
            if self.use_gpu:
                self.action_network.cuda()

            if self.deepen:
                self.deeper_action_network = load_prolonet(deep_act_fn, self.use_gpu)
                if self.use_gpu:
                    self.deeper_action_network.cuda()

            print('Model Loaded')
        else:
            raise Exception('No model found at ' + act_fn)
        return True
    ...
    def deepen_networks(self):
        ...
        for weight_index in range(len(self.action_network.layers)):
            new_act_weight = torch.Tensor(
                self.action_network.layers[weight_index].cpu().data.numpy())
            new_act_comp = torch.Tensor(
                self.action_network.comparators[weight_index].cpu().data.numpy(
                ))
            ...
            self.deeper_action_network.layers[
                weight_index].data = new_act_weight
            self.deeper_action_network.comparators[
                weight_index].data = new_act_comp
        for weight_index in range(len(self.value_network.layers)):
            new_val_weight = torch.Tensor(
                self.value_network.layers[weight_index].cpu().data.numpy())
            new_val_comp = torch.Tensor(
                self.value_network.comparators[weight_index].cpu().data.numpy(
                ))
            if self.use_gpu:
                new_val_weight = new_val_weight.cuda()
                new_val_comp = new_val_comp.cuda()
            self.deeper_value_network.layers[
                weight_index].data = new_val_weight
            self.deeper_value_network.comparators[
                weight_index].data = new_val_comp
    ...
    def entropy_leaf_checks(self):
        ...
        for leaf_index in range(len(self.action_network.action_probs)):
            existing_leaf = leaf_max(
                self.action_network.action_probs[leaf_index])
            new_leaf_1 = leaf_max(
                self.deeper_action_network.action_probs[2 * leaf_index + 1])
            new_leaf_2 = leaf_max(
                self.deeper_action_network.action_probs[2 * leaf_index])
            ...
            if new_entropy + 0.1 <= existing_entropy:
                with open('txts/' + self.bot_name + '_entropy_splits.txt',
                          'a') as myfile:
                    ...
                    myfile.write(
                        'Prior Probs: ' +
                        str(self.action_network.action_probs[leaf_index]) +
                        '\n')
                    myfile.write('New Probs 1: ' +
                                 str(self.deeper_action_network.action_probs[
                                     leaf_index * 2]) + '\n')
                    myfile.write('New Probs 2: ' +
                                 str(self.deeper_action_network.action_probs[
                                     leaf_index * 2 + 1]) + '\n')

                new_action_network = swap_in_node(new_action_network,
                                                  self.deeper_action_network,
                                                  leaf_index,
        ...
        if len(changes_made) > 0:
            ...
            if self.action_network.input_dim > 100:
                new_actor_opt = torch.optim.RMSprop(
                    self.action_network.parameters(), lr=1e-5)
            elif self.action_network.input_dim >= 8:
                new_actor_opt = torch.optim.RMSprop(
                    self.action_network.parameters(), lr=self.lr)
            else:
                new_actor_opt = torch.optim.RMSprop(
                    self.action_network.parameters(), lr=self.lr)
            ...
            for change in changes_made[::-1]:
                ...
                self.deeper_action_network = swap_in_node(
                    self.deeper_action_network,
                    None,
                    change * 2 + 1,
                    use_gpu=self.use_gpu)
