...
class ProLoNet(nn.Module):
    def __init__(
            ...
            leaves: t.
        Union[None, int,
              t.List],  # t.List[t.Tuple[t.List[int], t.List[int], np.array]]]
    ...
    def init_weights(self, weights):
        ...
        self.layers = nn.Parameter(
            new_weights, requires_grad=True
        )  # It should be number of nodes in the tree
    ...
    def init_paths(self):
        if type(self.leaf_init_information) is list:
            left_branches = torch.zeros(
                (len(self.layers), len(self.leaf_init_information)),
                dtype=torch.float)
            right_branches = torch.zeros(
                (len(self.layers), len(self.leaf_init_information)),
                dtype=torch.float)
            ...
            for n in range(0, depth):
                ...
                for i in range(0, 2**depth):
                    ...
                    if row + i >= len(left_branches) or end_col >= len(
                            left_branches[row]):
                        break
        ...
        if type(self.leaf_init_information) is list:
            ...
            for level in range(2**depth):
                ...
                new_probs = np.random.uniform(
                    0, 1, self.output_dim)  # *(1.0/self.output_dim)
                self.leaf_init_information.append(
                    [sorted(left_path),
                     sorted(right_path), new_probs])
